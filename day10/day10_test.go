...
func TestCreateLpProgram(t *testing.T) {
	machineDescription := "[.##.] (3) (1,3) (2) (2,3) (0,2) (0,1) {3,5,4,7}"
	got := CreateLpProgram(machineDescription)
	diff := cmp.Diff(programDescription, got)
	if diff != "" {
		t.Errorf("Contents mismatch (-want +got):\n%s", diff)
	}
}
